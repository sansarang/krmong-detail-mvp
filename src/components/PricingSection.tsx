...
export default function PricingSection({ lang = 'ko' }: { lang?: Lang }) {
  ...
  return (
    <section id="pricing" className="max-w-5xl mx-auto px-5 md:px-6 py-14 md:py-20">
      <div className="text-center mb-10 md:mb-14">
        <p className="text-xs font-black text-gray-300 uppercase tracking-widest mb-3">{lang === 'ko' ? '가격 플랜' : lang === 'ja' ? '料金プラン' : lang === 'zh' ? '价格方案' : 'Pricing'}</p>
        ...
        <div className="inline-flex items-center gap-3 bg-gray-100 rounded-2xl p-1.5 mt-4">
          <button onClick={() => setYearly(false)} className={`px-5 py-2 rounded-xl text-sm font-bold transition-all ${!yearly ? 'bg-white text-black shadow-sm' : 'text-gray-400'}`}>{L.monthly}</button>
            ...
            <div key={i} className={`relative rounded-3xl p-8 flex flex-col transition-all ${featured ? 'bg-black text-white shadow-2xl shadow-black/20 scale-[1.03]' : 'bg-white border border-gray-100 hover:border-gray-300 hover:shadow-lg'}`}>
              {plan.badge && (
                <div className="absolute -top-3.5 left-1/2 -translate-x-1/2">
                  <span className="bg-gradient-to-r from-orange-500 to-pink-500 text-white text-[11px] font-black px-4 py-1.5 rounded-full whitespace-nowrap shadow-lg">✦ {plan.badge}</span>
                </div>
              )}
              <div className="mb-6">
                <p className="text-xs font-black uppercase tracking-widest mb-2 text-gray-400">{plan.name}</p>
                <div className="flex items-end gap-1 mb-2">
                  <span className={`text-4xl md:text-5xl font-black tracking-[-0.04em] ${featured ? 'text-white' : 'text-black'}`}>{fmt(prices[i])}</span>
                  {prices[i] > 0 && <span className="text-sm font-medium mb-2 text-gray-400">{L.perMonth}</span>}
                </div>
                {yearly && prices[i] > 0 && (
      ...
      <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-6 text-sm text-gray-400">
        {[L.guarantee, L.noCard, L.cancel].map((t, i) => (
          <div key={i} className="flex items-center gap-2">
            <span className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-xs border ${i===0?'bg-green-50 border-green-100 text-green-500':i===1?'bg-blue-50 border-blue-100 text-blue-500':'bg-purple-50 border-purple-100 text-purple-500'}`}>✓</span>
...
}
  {
    ...
    features: [
      ...
      { text: 'SEO 분석 리포트',                included: false },
      { text: '네이버 블로그 1클릭 복사',         included: false },
      { text: '티스토리 자동 발행',              included: false },
  ...
  },
  {
    ...
    features: [
      ...
      { text: '6개 섹션 기본 템플릿',            included: true },
      { text: '인라인 편집',                    included: true },
      { text: 'SEO 분석 리포트',                included: true },
      { text: '네이버 블로그 1클릭 복사',         included: true },
      { text: '티스토리 자동 발행',              included: true },
