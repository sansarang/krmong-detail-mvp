...
export async function generateStaticParams() {
  return getMultilangPosts('en').map(p => ({ slug: p.slug }))
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const post = getMultilangPost('en', slug)
  if (!post) return {}
  return {
    title: `${post.title} | PageAI Blog`,
    description: post.description,
    keywords: post.tags.join(', '),
    alternates: { canonical: `https://pagebeer.beer/en/blog/${slug}` },
...
export default async function EnBlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const relatedPosts = getMultilangPosts('en').filter(p => p.slug !== post.slug).slice(0, 3)
        ...
        <nav className="border-b border-white/5 px-6 py-4 sticky top-0 bg-[#0B1120]/95 backdrop-blur z-10">
          <div className="max-w-3xl mx-auto flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <Link href="/login" className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg text-xs font-bold transition-colors">Start Free</Link>
            </div>
          </div>
        </nav>
        ...
        <article className="max-w-3xl mx-auto px-6 py-16">
          <header className="mb-12">
            <div className="flex items-center gap-2 mb-5">
              ...
              <span className="text-xs text-gray-500">{post.date} · {post.readTime} read</span>
            </div>
          ...
          <div className="mt-16 bg-gradient-to-r from-blue-600/20 to-violet-600/20 border border-blue-500/20 rounded-3xl p-10 text-center">
            <p className="text-xs font-bold text-gray-500 uppercase tracking-widest mb-3">Apply what you just learned</p>
            <h2 className="text-2xl font-black text-white mb-3">Generate your product page<br /><span className="text-gray-500">in 90 seconds.</span></h2>
            <p className="text-gray-500 text-sm mb-6">No credit card · Free to start · Amazon JP, Shopify, Tmall, Rakuten</p>
          ...
          <div className="mt-12">
            <p className="text-xs font-bold text-gray-600 uppercase tracking-widest mb-5">Related Articles</p>
