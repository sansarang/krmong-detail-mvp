...
export async function generateStaticParams() {
  return getMultilangPosts('ja').map(p => ({ slug: p.slug }))
}

export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params
  const post = getMultilangPost('ja', slug)
  if (!post) return {}
  return {
    title: `${post.title} | PageAI ブログ`,
    description: post.description,
    keywords: post.tags.join(', '),
    alternates: { canonical: `https://pagebeer.beer/ja/blog/${slug}` },
...
export default async function JaBlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const relatedPosts = getMultilangPosts('ja').filter(p => p.slug !== post.slug).slice(0, 3)
    ...
    <>
      ...
      <main className="min-h-screen bg-[#0B1120] text-white">
        <nav className="border-b border-white/5 px-6 py-4 sticky top-0 bg-[#0B1120]/95 backdrop-blur z-10">
          <div className="max-w-3xl mx-auto flex items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              <Link href="/blog" className="text-xs text-gray-400 hover:text-white transition-colors">← ブログ</Link>
              <Link href="/login" className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg text-xs font-bold transition-colors">無料で始める</Link>
            </div>
          </div>
        </nav>
        ...
        <article className="max-w-3xl mx-auto px-6 py-16">
          ...
          <div className="mt-12">
            <p className="text-xs font-bold text-gray-600 uppercase tracking-widest mb-5">関連記事</p>
            <div className="space-y-3">
              {relatedPosts.map(p => (
                <Link key={p.slug} href={`/ja/blog/${p.slug}`}
